...
class LaserScanVis:
  ...
  def update_scan(self):
    ...
    open3dPoints = open3d.utility.Vector3dVector(self.scan.points)
    ...
    for inst_label in unique_inst_labels:
      # print("Here ")
      if ((inst_label == 0) or (inst_label == 10)):
        continue
                # print("Inst label? ")
                # print(inst_label)
      # print("Here 1")
      ...
      if (semantic_label_for_inst == 10):
        # print("Here 4")

        points_for_inst = np.take_along_axis(points_for_scan, indices_for_inst, axis=0)
        if (points_for_inst.shape[0] >= 4):

        # print(self.scan.points)

            instPoints = open3d.utility.Vector3dVector(points_for_inst)
            bounding_box = open3d.geometry.OrientedBoundingBox.create_from_points(instPoints)
